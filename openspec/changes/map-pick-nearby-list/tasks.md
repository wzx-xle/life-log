...
- [ ] 5.3 拖动地图后列表刷新为新周边；取消不改动原地址（moveend 刷新代码在位；headless 连高德过慢无法稳定观测列表变化，待真机目视确认）
...
- [x] 6.3 瓦片请求大量"已取消"：`setCenter` 默认带平移动画，从默认中心飞到用户位置途中一路加载/取消瓦片。改 `setCenter([lng,lat], true)` 关动画直接跳（真机待确认churn下降）
